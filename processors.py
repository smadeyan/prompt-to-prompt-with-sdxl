...
class AttendExciteCrossAttnProcessor:
    ...
    def __init__(self, attnstore, place_in_unet, use_p2p=False):
        super().__init__()
    ...
    def __call__(self, attn: Attention,hidden_states: torch.FloatTensor,encoder_hidden_states = None,
        ...
        # Add P2P attention modification
        if is_cross and self.use_p2p and hasattr(self.attnstore, 'mapper'):
            h = attention_probs.shape[0] // 2
            attn_base, attn_replace = attention_probs[:h], attention_probs[h:]
            
            # Apply P2P refinement
            attn_base_replace = attn_base[:, :, self.attnstore.mapper].permute(2, 0, 1, 3)
            attn_replace = attn_base_replace * self.attnstore.alphas + attn_replace * (1 - self.attnstore.alphas)
            attention_probs[h:] = attn_replace
